...
import { TransactionFrequencyByMerchantResponse } from '@/lib/api'
import { formatCurrency } from '@/lib/formatters'
import { EmptyState } from './EmptyState'
import { useIsMobile } from '@/hooks/useIsMobile'

interface TransactionFrequencyByMerchantProps {
  data: TransactionFrequencyByMerchantResponse | undefined
  isLoading: boolean
  dateRange?: string
}

export function TransactionFrequencyByMerchant({ data, isLoading, dateRange }: TransactionFrequencyByMerchantProps) {
  ...
  if (isLoading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Transaction Frequency by Merchant</CardTitle>
          <CardDescription>Average transactions per month by merchant</CardDescription>
        </CardHeader>
        <CardContent>
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Transaction Frequency by Merchant</CardTitle>
        <CardDescription>
          Average transactions per month by merchant
          {dateRange && ` - ${dateRange}`}
        </CardDescription>
      </CardHeader>
      <CardContent>
                ...
                if (active && payload && payload.length > 0) {
                  ...
                  return (
                    <div className="max-w-[260px] rounded-lg border border-border bg-background p-3 shadow-lg sm:max-w-none">
                      ...
                      <p className="text-xs text-muted-foreground">{`${formatCurrency(row.total_amount)} total spent`}</p>
                    </div>
                  )
                }
                return null
